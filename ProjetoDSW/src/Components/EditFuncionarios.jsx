...
const EditFuncionarios = () => {
    ...
    const [employee, setFuncionario] = useState({
      ...
      const [category, setCargos] = useState([])
      ...
      useEffect(() => {
        axios.get('http://localhost:3000/auth/cargos')
            ...
            axios.get('http://localhost:3000/auth/funcionarios/' + id)
            .then(result => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        axios.put('http://localhost:3000/auth/edit_funcionarios/' + id, employee)
        .then(result => {
            if(result.data.Status) {
  ...
  return (
    <div className="d-flex justify-content-center align-items-center mt-3">
      <div className="p-3 rounded w-50 border">
        <h3 className="text-center">Editar Funcionario</h3>
